...
#
# This script generates the Refresh token for oac-create-snapshot.sh consumption, No need to call explicitly.
#
#====================================================
...
then
 ...
 echo $TOKENS | sed 's/^.*refresh_token":"//' | sed 's/"}//' |awk '{print $0}' > $PWD/source_refreshToken.txt
 echo "New Refresh Token saved to the source_refreshToken.txt file is as below:"
 echo $(cat $PWD/source_refreshToken.txt)
fi
}
